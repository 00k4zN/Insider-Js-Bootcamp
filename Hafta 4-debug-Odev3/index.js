...
class ShoppingCart {
    ...
    addItem(productId, quantity = 1) {
        try {
            ...
            if (product.stock < quantity) { // Düzeltme: <= yerine < kullanıldı
    ...
    removeItem(productId) {
        try {
            ...
            if (product) {
                product.stock += item.quantity; // Düzeltme: Sabit değer yerine item.quantity
            }
    ...
    calculateTotal() {
        this.total = this.items.reduce((sum, item) => {
            return sum + (item.price * item.quantity); // Düzeltme: Miktar ile çarpmayı ekledik
        }, 0);

        if (this.discountApplied && this.total > 0) {
            this.total *= 0.9; // Düzeltme: 0.1 yerine 0.9 ile çarptık (%10 indirim)
        }
    }
    ...
    showError(message) {
        ...
        if (errorElement) {
            errorElement.textContent = message; // += yerine = kullanarak arka arkaya yazılan hata mesajını engelledim.
            
            // Hata mesajını belirli bir süre sonra temizleme
            setTimeout(() => {
...
class App {
    ...
    addToCart(productId) {
        window.cart.addItem(productId, 1); // Düzeltme: undefined yerine 1 değeri
